...
;; added by chrdimo
;; opt/future-pred helper
;;
(define-for-syntax (opt/future-pred opt/info pred)
  (with-syntax ((pred pred))
    (values
     (with-syntax ((val (opt/info-val opt/info))
                   (ctc (opt/info-contract opt/info))
...
;; added by chrdimo
;; fututre-flat-contract helper
;;
(define-for-syntax (opt/future-flat-ctc opt/info pred checker)
  (syntax-case pred (null? number? integer? boolean? pair? not)
    ;; Better way of doing this?
    [null? (opt/pred opt/info pred)]
    [number? (opt/pred opt/info pred)]
